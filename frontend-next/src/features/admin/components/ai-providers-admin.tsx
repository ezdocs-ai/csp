...
export function formatProviderType(type: string): string {
  switch (type.toLowerCase()) {
    case "google_vertex": return "Google Vertex AI";
    case "google_ai": return "Google AI Studio";
    case "replicate": return "Replicate";
    case "fal": return "fal.ai";
    case "ark": return "BytePlus Ark";
  ...
  return (
    <section className="space-y-[var(--tri-space-6)]">
          ...
          <TableBody>
            {list.length === 0 ? (
              ...
              <TableRow key={provider.id}>
                ...
                <TableCell><Badge tone="info">{formatProviderType(provider.providerType)}</Badge></TableCell>
      ...
      <div className="md:hidden">
        {list.length === 0 ? (loading ? <div aria-busy="true" aria-label="Loading AI providers" className="grid gap-[var(--tri-space-2)]">{Array.from({ length: 2 }, (_, index) => <div className="h-28 animate-pulse rounded-[var(--tri-card-radius)] bg-[var(--tri-bg-surface-alt)]" key={index} />)}</div> : <EmptyState actions={<Button onClick={() => setEditing({ form: { ...EMPTY_FORM } })}>Add provider</Button>} description="Add a provider registry entry to connect vendor models." title="No AI providers found" />) : <ul className="grid gap-[var(--tri-space-3)]">{list.map((provider) => <li className="grid gap-[var(--tri-space-3)] rounded-[var(--tri-card-radius)] border border-[var(--tri-border-subtle)] bg-[var(--tri-bg-surface)] p-[var(--tri-space-4)]" key={provider.id}><div className="flex items-start justify-between gap-[var(--tri-space-3)]"><div className="min-w-0"><p className="font-[var(--tri-font-weight-semibold)] text-[var(--tri-text-primary)]">{provider.displayName}</p><p className="font-mono text-[length:var(--tri-text-small-size)] text-[var(--tri-text-tertiary)]">{provider.key}</p></div><Badge tone="info">{formatProviderType(provider.providerType)}</Badge></div><div className="flex flex-wrap gap-[var(--tri-space-2)]"><Badge tone={provider.hasSecret ? "success" : "neutral"}>{provider.hasSecret ? "Secret configured" : "Secret not set"}</Badge><span className="inline-flex items-center text-[length:var(--tri-text-small-size)] text-[var(--tri-text-secondary)]">Timeout {provider.timeoutSeconds}s</span></div><div className="flex items-center justify-between gap-[var(--tri-space-2)] border-t border-[var(--tri-table-row-divider)] pt-[var(--tri-space-3)]"><SlideToggle checked={provider.enabled} label={`Toggle ${provider.displayName}`} onChange={(enabled) => void toggleEnabled(provider, enabled)} /><div className="flex flex-wrap justify-end gap-[var(--tri-space-1)]"><Button onClick={() => setEditing({ id: provider.id, form: providerToInput(provider) })} variant="secondary">Edit</Button><Button disabled={busy === provider.id} onClick={() => void runTest(provider.id)} variant="ghost">{busy === provider.id ? "Testing…" : "Test"}</Button><Button onClick={() => setDeleting(provider)} variant="ghost">Delete</Button></div></div></li>)}</ul>}
      </div>
