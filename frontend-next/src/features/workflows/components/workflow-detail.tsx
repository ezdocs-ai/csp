...
import { Badge, Button, EmptyState, LoadingState } from "@/src/components/ui";
...
export function WorkflowDetail({ workflow, canEdit = false }: { workflow: Workflow; canEdit?: boolean }) {
  ...
  return (
    <section aria-label="Workflow detail" className="mx-auto max-w-[var(--tri-layout-wide)] space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-[var(--tri-space-1)] text-[length:var(--tri-text-h2-size)] leading-[var(--tri-text-h2-line-height)] tracking-[var(--tri-text-h2-tracking)] font-[var(--tri-font-weight-semibold)]">{workflow.name}</h1>
      ...
      {loading && executions.length === 0 ? <LoadingState label="Loading executions" /> : null}
      ...
      {!loading && executions.length === 0 && !error ? (
        ...
        <ul className="grid gap-2">
          {executions.map((execution) => (
            <li key={execution.id}>
              <button className="flex w-full items-center justify-between gap-4 rounded-[var(--tri-card-radius)] border border-[var(--tri-card-border)] bg-[var(--tri-card-bg)] p-[var(--tri-space-3)] text-left hover:border-[var(--tri-card-interactive-hover-border)]" onClick={() => setSelected(execution)} type="button">
